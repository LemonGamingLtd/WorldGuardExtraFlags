...
public class FlyFlagHandler extends FlagValueChangeHandler<State> {
	...
	private void handleValue(LocalPlayer player, World world, State state) {
		...
		if (!bukkitPlayer.isOnline() || !bukkitPlayer.isValid() || bukkitPlayer.isDead()) {
			return;
		}
		...
		WorldGuardUtils.getScheduler().getScheduler().runAtEntity(bukkitPlayer, wrappedTask -> task.run());
	}
}
